...
function parsePostJson(json) {
  ...
  const actor = storyNode?.actors?.[0] ||
                storyNode?.comet_sections?.content?.story?.comet_sections?.context_layout?.story?.comet_sections?.actor_photo?.story?.actors?.[0];

  const postText = storyNode?.message?.text ||
                   storyNode?.comet_sections?.content?.story?.comet_sections?.message_container?.story?.message?.text;

  const imageText = storyNode?.attachments?.[0]?.styles?.attachment?.media?.accessibility_caption;

  const postId = storyNode?.post_id ||
                 storyNode?.comet_sections?.feedback?.story?.post_id;
  ...
  const post = {
    ...
    email: getEmailFromText(postText || '') || getEmailFromText(imageText || ''),
